function [comEqualizer, priEqualizer, comWeight, priWeight, comRate, priRate] = rs_terms(bcChannel, comPrecoder, priPrecoder, order)
% Function:
...
%
% Comment(s):
%   - for multi layer ordered RS on MU-MISO systems only
%
% Reference(s):
...
for iUser = 1 : user
    for iLayer = 1 : user
        if iLayer ~= 1
            ...
            priPowTerm(iUser, iLayer) = priPowTerm(iUser, iLayer - 1) - abs(bcChannel(:, iUser)' * priPrecoder(:, order(iLayer - 1))) .^ 2;
        end
        % interference power at the i-th layer (when decoding common stream)
        intPowTerm(iUser, iLayer) = priPowTerm(iUser, iLayer) - abs(bcChannel(:, iUser)' * priPrecoder(:, order(iLayer))) .^ 2;
    end
end

% SIC stops after users decoding own layer
priPowTerm(order, :) = tril(priPowTerm(order, :));
...
% private rate (no influence from common part; rate of each layer must be achievable for all users)
